...
class CognitoUtilities:
    ...
    def signup_user(self, email, password, user_pool_id=None):
        assert user_pool_id is not None, "You need to send user_pool_id in this function in order to auto-verify user on sign-up. Else, this practice is not recommended!"
        ...
        try:
            ...
            print("{0}\n{1}".format(e, traceback.format_exc()))
            status, message, user_id = 409, "{0}\nPassword minimum length should be 8 character(s)\n".format(e.response["Error"]["Message"]), None
        except Exception as e:
            print("{0}\n{1}".format(e, traceback.format_exc()))
