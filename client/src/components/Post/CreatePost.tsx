...
const CreatePost = () => {
  ...
  // Declaring all the field's variables and erros pertaining to that variables.
  ...
  const [errorsindustry, setErrorsindustry] = React.useState<{
    name: string;
  }>();

  const [companyInfo, setCompnayInfo] = React.useState<string>();
  const [errorscompanyinfo, setErrorscompanyinfo] = React.useState<{
    name: string;
  }>();
  ...
  const [errorstechnology, setErrorstechnology] = React.useState<{
    name: string;
  }>();

  const [investors, setInvestors] = React.useState<string>();
  const [errorsinvestors, setErrorsinvestors] = React.useState<{
    name: string;
  }>();

  const [founders, setFounders] = React.useState<string>();
  const [errorsfounders, setErrorsfounders] = React.useState<{
    name: string;
  }>();

  const [foundersInfo, setFoundersInfo] = React.useState<string>();
  const [errorsfoundersinfo, setErrorsfoundersinfo] = React.useState<{
    name: string;
  }>();

  const [foundedIn, setFoundedIn] = React.useState<string>();
  const [errorsfoundedin, setErrorsfoundedin] = React.useState<{
    name: string;
  }>();

  const [headquarters, setHeadquarters] = React.useState<string>();
  const [errorsheadquarters, setErrorsheadquarters] = React.useState<{
    name: string;
  }>();
  ...
  const [valuation, setValuation] = React.useState<string>();
  const [errorsvaluation, setErrorsvaluation] = React.useState<{
    name: string;
  }>();

  const [employees, setEmployees] = React.useState<string>();
  const [errorsemployees, setErrorsemployees] = React.useState<{
    name: string;
  }>();

  const [teamInfo, setteamInfo] = React.useState<string>();
  const [errorsteaminfo, setErrorsteaminfo] = React.useState<{
    name: string;
  }>();

  const [funding, setFunding] = React.useState<string>();
  const [errorsfunding, setErrorsfunding] = React.useState<{ name: string }>();

  const [fundingSeeking, setFundingSeeking] = React.useState<string>();
  const [errorsfundingseeking, setErrorsfundingseeking] = React.useState<{
    name: string;
  }>();
  ...
  // Function is called when the user clicks add post button.
  const postClick = () => {
    ...
    standardString = typeof value === "string" ? value.trim() : "";
    if (standardString.length === 0) {
      setErrorsfundingseeking({ name: "Funding seeking information cannot be empty" });
      errorState = errorState + 1;
    }
    ...
    if (errorState === 0) {
      ...
      httpClient
        ...
        .then((response: any) => {
          if (response.data.message === "Post details inserted successfully!") {
  ...
  // If user chooses to discard the post, it will navigate to the feeds page.
  const discardClick = () => {
    navigate("/feeds");
  };
  ...
  const handleChangeInCompany = (event: React.ChangeEvent<HTMLInputElement>) => {
    const {
      target: { value },
    } = event;
    setCompnay(value);
  ...
  const handleChangeInInvestors = (event: React.ChangeEvent<HTMLInputElement>) => {
    const {
      target: { value },
    } = event;
    setInvestors(value);
